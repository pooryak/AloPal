...
import { ApiMessageListCorrespondentsRepository, ApiMessageListMeetingRepository } from 'repository';
...
function Messages(props) {
    ...
    //     },
    // });
    const getMessages = useMutation((user) => ApiMessageListMeetingRepository.apiMessageListMeetingPost(user), {
        onSuccess: (apiData) => {
            const decodedData = decoder.alopal.backend.ListMessagesResponse.decode(apiData.data);
            console.log('🚀 ~ file: index.js ~ line 115 ~ registerMutation ~ decodedData', decodedData);
            setCurrentUser(decodedData.messages);
    ...
    const getCorrespondants = useMutation((user) => ApiMessageListCorrespondentsRepository.apiMessageListCorrespondentsPost(user), {
        onSuccess: (apiData) => {
            const decodedData = decoder.alopal.backend.ListCorrespondantsResponse.decode(apiData.data);
            console.log('🚀 ~ file: index.js ~ line 115 ~ registerMutation ~ decodedData', decodedData);
            // setCurrentUser(decodedData.messages);
        },
        onError: (error) => {
    ...
    const fetchApi = async () => {
        ...
        const verify = decoder.alopal.backend.ListMessageForMeeting.create(dataWithContext);
        const encodedData = decoder.alopal.backend.ListMessageForMeeting.encode(verify).finish();
    ...
    const fetchCorrespondants = async () => {
        ...
        const verify = decoder.alopal.backend.ListCorrespondantsRequest.create(dataWithContext);
        const encodedData = decoder.alopal.backend.ListCorrespondantsRequest.encode(verify).finish();
        const datacv = await getCorrespondants.mutateAsync(encodedData);
    };
    useEffect(() => {
    ...
    return (
        <Paper elevation={1} className={classes.root}>
            <Grid container className={classes.container}>
                ...
                <Grid item xs={9}>
                    <ChatBox data={currenUser} refresh={() => fetchApi()} />
                </Grid>
            </Grid>
        </Paper>
